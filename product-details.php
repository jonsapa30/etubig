	...
	if(isset($_SESSION['cart'][$id])){
		...
		$row = mysqli_fetch_assoc($res); 
			...
			if ($row['REMAINING'] < $_SESSION['cart'][$id]['quantity']) {
				...
				echo '<script>window.location="product-details.php?pid='.$id.'"</script>';
			} 
		}else{
   ...
   if(isset($_SESSION['cart'][$id])){
	   ...
	   $row = mysqli_fetch_assoc($res); 
		   ...
		   if(mysqli_num_rows($query_p)!=0){
			   $row_p=mysqli_fetch_array($query_p);
			   $_SESSION['cart'][$row_p['id']]=array("quantity" => $r_qty, "price" => $row_p['productPrice']);
			   header('location:exchange_bottle.php');
		   }else{
			   $message="Product ID is invalid";
		   }
	   }
...
<?php
$ret=mysqli_query($conn,"select category.categoryName as catname,subCategory.subcategory as subcatname,products.productName as pname from products join category on category.id=products.category join subcategory on subcategory.id=products.subCategory where products.id='$pid'");
						...
						<div class="product-info text-left m-t-20">
							...
							<div class="product-price" style="color:grey;">	
								<span class="price" style="color:grey;">
									New: ₱<?php echo htmlentities($rws['productPrice']);?>.00 | Exchange: ₱<?php echo htmlentities($rws['productPriceBeforeDiscount']);?>.00
								</span>
...
<div class="stock-container info-container m-t-10">
							...
							<div class="price-container info-container m-t-20">
								<div class="row">
									...
									<div class="col-sm-6">
										<div class="price-box">
											<span class="price" style="color:grey;">₱ <?php echo htmlentities($row['productPrice']);?>.00</span>| New Bottle
											<!---<span class="price-strike">₱<?php // echo htmlentities($row['productPriceBeforeDiscount']);?></span>-->
											<span class="price" style="color:grey;">₱ <?php echo htmlentities($row['productPriceBeforeDiscount']);?>.00</span>| Exchange
										</div>
									</div>
									<br>
...
<form action="product-details.php?page=product&action=add&id=<?php echo $row['id']; ?>" method="POST">
							...
							<div class="quantity-container info-container">
								<div class="row">
									...
									<div class="col-sm-7">
										<button type="submit" name="savetocart" class="btn btn-primary"><i class="fa fa-shopping-cart inner-right-vs"></i> NEW BOTTLE</button>
										<button style="margin-left:14rem; margin-top:-5rem;" type="submit" name="exchange" class="btn btn-primary"><i class="fa fa-recycle inner-right-vs"></i> EXCHANGE BOTTLE</button>
									</div>
...
<?php $qry=mysqli_query($conn,"select * from productreviews where productId='$pid'");
...
<!--<section class="section featured-product wow fadeInUp">
	...
	<div class="owl-carousel home-owl-carousel upsell-product custom-carousel owl-theme outer-top-xs">
		...
		<?php //$qry=mysqli_query($conn,"select * from products where subCategory='$subcid' and category='$cid'"); 
