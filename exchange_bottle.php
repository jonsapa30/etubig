...
foreach($value as $qty=> $val34){
				...
				mysqli_query($conn,"insert into orders(userId,productId,quantity,orderStatus,type) values('".$_SESSION['id']."','$qty','$val34','Pending', '1')");
 ...
 $pdtid=array();
    $sql = "SELECT * FROM products WHERE id IN(";
			...
			while($row = mysqli_fetch_array($query)){
				...
				$subtotal= $_SESSION['cart'][$row['id']]['quantity']*$row['productPriceBeforeDiscount']+($row['shippingCharge'] * $_SESSION['cart'][$row['id']]['quantity']);
				$totalprice += $subtotal;
...
$_SESSION['sid']=$pd;
						...
						<div class="row">
							...
							<div class="col-sm-8">
<?php $rt=mysqli_query($conn,"select * from productreviews where productId='$pd'");
...
?>
					...
					<td class="cart-product-quantity">
						...
						<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "₱"." ".$row['productPriceBeforeDiscount']; ?>.00</span></td>
						
					<td class="cart-product-sub-total"><span class="cart-sub-total-price"><?php echo "₱"." ".$row['shippingCharge']; ?>.00</span></td>

					<td class="cart-product-grand-total"><span class="cart-grand-total-price"><?php echo "₱"." ".($_SESSION['cart'][$row['id']]['quantity']*$row['productPriceBeforeDiscount']+ ($row['shippingCharge'] * $_SESSION['cart'][$row['id']]['quantity'])); ?>.00</span></td>
				</tr>
					...
					<td>
						<div class="form-group">
							<?php
								 ...
								 $qry=mysqli_query($conn,"select * from users where id='".$_SESSION['id']."'");
								 $rt=mysqli_fetch_assoc($qry);

								 	if ($rt['shippingAddress']=='' ) {
								 		# code...
								 		echo '<textarea class="form-control" name="shippingAddress" required></textarea>';
								 	}else{
	...
	<table class="table table-bordered">
		...
		<tbody>
			<!---	<tr>
					<td>
						<div class="form-group">
						<?php // $qry=mysqli_query($conn,"select * from users where id='".$_SESSION['id']."'");
 						//	$rt=mysqli_fetch_assoc($qry);
 							//if ($rt['billingAddress']=='') {
 								# code...
							//	 echo '<textarea class="form-control" name="billingAddress" required></textarea>';
 							//}else{
			...
			<tr>
				<th>
					...
					<div class="cart-grand-total" style="color:black;">
						Grand Total<span class="inner-left-md"><?php echo "P". " ". $_SESSION['tp']="$totalprice". ".00"; ?></span>
					</div>
				</th>
			</tr>
		</thead><!-- /thead -->
