...
if (strlen($_SESSION['login']) == 0) {   
    ...
    if (isset($_GET['action']) && $_GET['action'] == "add") {
        $id = intval($_GET['id']);
        $stmt = $conn->prepare("DELETE FROM wishlist WHERE productId = ?");
...
<div class="body-content outer-top-bd">
	...
	<div class="table-responsive">
		<table class="table">
			...
			<tbody>
<?php
$ret=mysqli_query($conn,"select products.productName as pname,products.productImage1 as pimage,products.productPrice as pprice,wishlist.productId as pid,wishlist.id as wid from wishlist join products on products.id=wishlist.productId where wishlist.userId='".$_SESSION['id']."'");
