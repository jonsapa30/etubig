...
foreach($value as $qty=> $val34){
	...
	mysqli_query($conn,"insert into orders(userId,productId,quantity,orderStatus,type) values('".$_SESSION['id']."','$qty','$val34','Pending', '0')");
 ...
 $pdtid=array();
    $sql = "SELECT * FROM products WHERE id IN(";
			...
			while($row = mysqli_fetch_array($query)){
				...
				$subtotal= $_SESSION['cart'][$row['id']]['quantity']*$row['productPrice']+($row['shippingCharge'] * $_SESSION['cart'][$row['id']]['quantity']);
				$totalprice += $subtotal;
...
?>
					...
					<td class="cart-product-grand-total"><span class="cart-grand-total-price"><?php echo "₱"." ".($_SESSION['cart'][$row['id']]['quantity']*$row['productPrice']+ ($row['shippingCharge'] * $_SESSION['cart'][$row['id']]['quantity'])); ?>.00</span></td>
				</tr>
